...
class RoutingMapMultiLaneView extends StatefulWidget {
  /// The stream that receives notifications when the bottom sheet is dragged.
  final Stream<DraggableScrollableNotification>? sheetMovement;

  const RoutingMapMultiLaneView({required this.sheetMovement, Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => RoutingMapMultiLaneViewState();
}

class RoutingMapMultiLaneViewState extends State<RoutingMapMultiLaneView> with TickerProviderStateMixin {
  static const viewId = "routing.views.map.multi_lane";
  ...
  void updateMap() {
    ...
    if (routingMultiLane.needsLayout[viewId] != false) {
  ...
  fitCameraToRouteBounds() async {
    ...
    final cameraOptionsForBounds = await mapController?.cameraForCoordinateBounds(
      routingMultiLane.selectedRoute!.paddedBounds,
  ...
  onFeatureTapped(QueriedFeature queriedFeature) async {
    ...
    if ((id as String).startsWith("route-")) {
      ...
      routingMultiLane.switchToRoute(routeIdx);
  ...
  onMapLongClick(BuildContext context, double x, double y) async {
    ...
    String fallback = "Wegpunkt ${(routingMultiLane.selectedWaypoints?.length ?? 0) + 1}";
    ...
    if (routingMultiLane.selectedWaypoints == null || routingMultiLane.selectedWaypoints!.isEmpty) {
      await routingMultiLane
          .addWaypoint(Waypoint(positioning.lastPosition!.latitude, positioning.lastPosition!.longitude));
    }
    await routingMultiLane.addWaypoint(Waypoint(latitude, longitude, address: address));
