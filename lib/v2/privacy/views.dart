  ...
  Widget renderPrivacyPolicy(PrivacyPolicy policy) {
    return Stack(alignment: Alignment.bottomCenter, children: [
      HPad(child: Fade(child: ListView(children: [
        ...
        if (!policy.hasChanged) const Header(text: "Diese App funktioniert mit"),
        if (!policy.hasChanged) const ColorHeader(text: "deinen Daten."),
        if (policy.hasChanged) const Header(text: "Wir haben die Erklärung zum"),
        if (policy.hasChanged) const ColorHeader(text: "Datenschutz aktualisiert."),
        const SmallVSpace(),
        if (!policy.hasChanged) const SubHeader(text: "Bitte lies dir deshalb kurz durch, wie wir deine Daten schützen. Das Wichtigste zuerst:"),
        if (policy.hasChanged) const SubHeader(text: "Lies dir hierzu kurz unsere Änderungen durch."),
        const VSpace(),
