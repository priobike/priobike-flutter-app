...
class Profile extends ChangeNotifier {
  ...
  /// Convert a json map to a profile.
  factory Profile.fromJson(Map<String, dynamic> jsonData) {
    return Profile(
      bikeType: jsonData["bikeType"] != null ? BikeType.values.byName(jsonData["bikeType"]) : null,
      preferenceType: jsonData["preferenceType"] != null ? PreferenceType.values.byName(jsonData["preferenceType"]) : null,
      activityType: jsonData["activityType"] != null ? ActivityType.values.byName(jsonData["activityType"]) : null,
    );
  }

  /// The key under which the model is stored in the user defaults / shared preferences.
  static const key = "priobike.home.profile";

  /// Load the stored profile.
  static Future<Profile> load() async {
    final storage = await SharedPreferences.getInstance();
    final json = storage.getString(key);
    // If we have no profile, we create a new one.
  ...
  Future<void> store() async {
    final storage = await SharedPreferences.getInstance();
    final parsedJson = jsonEncode(toJSON());
    await storage.setString(key, parsedJson);
    notifyListeners();
  }

  /// Convert the profile to a json map.
  Map<String, dynamic> toJSON() {
    return {
      if (bikeType != null) "bikeType": bikeType!.name,
      if (preferenceType != null) "preferenceType": preferenceType?.name,
      if (activityType != null) "activityType": activityType?.name,
    };
  }
}