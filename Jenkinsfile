        ...
        stage('Build and test Flutter app') {
            steps {
                script {
                    ...
                    sh 'docker run --rm -v ${PWD}/.netrc:${HOME}/.netrc -v ${PWD}:/app -w /app cirrusci/flutter:3.0.1 /bin/bash -c "flutter pub get && flutter test && flutter build apk --dart-define=COMMIT_ID=$(git rev-parse --short HEAD~)"'
